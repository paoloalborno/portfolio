...
const translations = {
    ...
    "project_sql_toolkit": {
        ...
        "goal_h3": { "en": "Project Goal", "it": "Obiettivo del Progetto", "fr": "Objectif du Projet" },
        "goal_p": { "en": "This SQL Reverse Engineering Toolkit is a command-line application designed to analyze and visualize data lineage in a MySQL database. It extracts stored procedures and table schemas, parses them to identify dependencies (which procedures read from or write to which tables), and generates an interactive dependency graph. This tool is ideal for understanding complex database logic, refactoring legacy code, and documenting data workflows.<br><br>Key features include <strong>Database Extraction</strong> of `CREATE TABLE` statements and stored procedures, <strong>SQL Parsing</strong> to identify read/write operations, and <strong>Graph Visualization</strong> with Graphviz to map data flows. It also supports creating and managing <strong>materialized views</strong>.<br><br>The application is divided into distinct modules (extractor, parser, graph_utils, materializer), each with a specific responsibility. This modularity, following the <strong>Separation of Concerns</strong> principle, makes the code easier to understand, maintain, and extend. Database credentials and other settings are managed through environment variables (<code>.env</code> file), separating configuration from code and enhancing security. The use of <code>argparse</code> provides a user-friendly CLI, making the tool accessible and scriptable.", "it": "Questo SQL Reverse Engineering Toolkit è un'applicazione a riga di comando progettata per analizzare e visualizzare la data lineage in un database MySQL. Estrae stored procedure e schemi di tabelle, li analizza per identificare le dipendenze (quali procedure leggono o scrivono su quali tabelle) e genera un grafo di dipendenza interattivo. Questo strumento è ideale per comprendere logiche di database complesse, effettuare il refactoring di codice legacy e documentare i flussi di dati.<br><br>Le funzionalità principali includono l'<strong>Estrazione dal Database</strong> degli statement `CREATE TABLE` e delle stored procedure, il <strong>Parsing SQL</strong> per identificare le operazioni di lettura/scrittura e la <strong>Visualizzazione a Grafo</strong> con Graphviz per mappare i flussi di dati. Supporta anche la creazione e gestione di <strong>viste materializzate</strong>.<br><br>L'applicazione è suddivisa in moduli distinti (extractor, parser, graph_utils, materializer), ognuno con una responsabilità specifica. Questa <strong>modularità</strong>, seguendo il principio della <strong>Separazione delle Competenze</strong>, rende il codice più facile da capire, mantenere ed estendere. Le credenziali del database e altre impostazioni sono gestite tramite variabili d'ambiente (file <code>.env</code>), separando la configurazione dal codice e migliorando la sicurezza. L'uso di <code>argparse</code> fornisce una CLI user-friendly, rendendo lo strumento accessibile e scriptabile.", "fr": "Cet Outil de Rétro-ingénierie SQL est une application en ligne de commande conçue pour analyser et visualiser la lignée des données dans une base de données MySQL. Il extrait les procédures stockées et les schémas de table, les analyse pour identifier les dépendances (quelles procédures lisent ou écrivent dans quelles tables) et génère un graphe de dépendances interactif. Cet outil est idéal pour comprendre la logique complexe des bases de données, remanier du code hérité et documenter les flux de données.<br><br>Les fonctionnalités clés incluent l'<strong>Extraction de base de données</strong> des instructions `CREATE TABLE` et des procédures stockées, l'<strong>Analyse SQL</strong> pour identifier les opérations de lecture/écriture, et la <strong>Visualisation de graphe</strong> avec Graphviz pour cartographier les flux de données. Il prend également en charge la création et la gestion de <strong>vues matérialisées</strong>.<br><br>L'application est divisée en modules distincts (extracteur, analyseur, graph_utils, matérialiseur), chacun ayant une responsabilité spécifique. Cette <strong>modularité</strong>, suivant le principe de la <strong>Séparation des Préoccupations</strong>, rend le code plus facile à comprendre, à maintenir et à étendre. Les informations d'identification de la base de données et d'autres paramètres sont gérés via des variables d'environnement (fichier <code>.env</code>), séparant la configuration du code et améliorant la sécurité. L'utilisation d'<code>argparse</code> fournit une CLI conviviale, rendant l'outil accessible et scriptable." },
