...
const translations = {
    ...
    "project_sql_toolkit": {
        ...
        "goal_h3": { "en": "Project Goal", "it": "Obiettivo del Progetto", "fr": "Objectif du Projet" },
        "goal_p": { "en": "This SQL Reverse Engineering Toolkit is a command-line application designed to dissect and clarify the intricate data lineage within a MySQL database. In complex systems where data flows through numerous stored procedures and tables, understanding the relationships between them is crucial for maintenance, optimization, and refactoring. The tool automates the process by extracting `CREATE TABLE` statements and stored procedure definitions, parsing the SQL code to map out dependencies, and generating an intuitive, interactive dependency graph. It is particularly valuable for modernizing legacy systems, enabling developers to visualize the impact of changes, identify unused database objects, and produce clear documentation of the data architecture. This empowers teams to make informed decisions, reduce the risk of breaking changes, and streamline the database development lifecycle.", "it": "Questo SQL Reverse Engineering Toolkit è un'applicazione a riga di comando progettata per sezionare e chiarire l'intricata data lineage all'interno di un database MySQL. In sistemi complessi, dove i dati fluiscono attraverso numerose tabelle e stored procedure, comprendere le relazioni tra di esse è fondamentale per la manutenzione, l'ottimizzazione e il refactoring. Lo strumento automatizza questo processo estraendo le istruzioni `CREATE TABLE` e le definizioni delle stored procedure, analizzando il codice SQL per mappare le dipendenze e gerando un grafo delle dipendenze intuitivo e interattivo. È particolarmente prezioso per modernizzare sistemi legacy, consentendo agli sviluppatori di visualizzare l'impatto delle modifiche, identificare oggetti di database inutilizzati e produrre una chiara documentazione dell'architettura dei dati. Ciò consente ai team di prendere decisioni informate, ridurre il rischio di introdurre errori e ottimizzare il ciclo di vita dello sviluppo del database.", "fr": "Cet Outil de Rétro-ingénierie SQL est une application en ligne de commande conçue pour disséquer et clarifier la lignée complexe des données au sein d'une base de données MySQL. Dans les systèmes où les données transitent par de nombreuses procédures stockées et tables, la compréhension de leurs interrelations est cruciale pour la maintenance, l'optimisation et la refonte. L'outil automatise ce processus en extrayant les instructions `CREATE TABLE` et les définitions de procédures stockées, en analysant le code SQL pour cartographier les dépendances, et en générant un graphe de dépendances intuitif et interactif. Il est particulièrement utile pour moderniser les systèmes hérités, permettant aux développeurs de visualiser l'impact des changements, d'identifier les objets de base de données inutilisés et de produire une documentation claire de l'architecture des données. Cela permet aux équipes de prendre des décisions éclairées, de réduire le risque de modifications disruptives et de rationaliser le cycle de vie du développement de la base de données." },
